...
Uses family-owned graph primitives and TensorRT APIs only. EAGLE3 topology
follows TensorRT-Edge-LLM e8b29522938901f6df19ebeedd4b69bc8edbcd97;
no Edge-LLM module, exporter, plugin, or build dependency is imported.
"""
...
class Graph:
    ...
    def attention(self, value, weights, prefix, layer):
        ...
        # inputs/outputs. A plugin must implement the same append and visibility.
...
def build_target(config, weights, contract, *, verbose=False):
    ...
    features = []
    # Edge's all_hidden_states indexes the inputs to layers (embedding is 0).
