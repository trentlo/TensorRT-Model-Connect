...
T require_value(const nlohmann::json& json, const char* name) {
    if (!json.contains(name))
        throw std::runtime_error(std::string("llama runtime.json missing '") + name + "'");
    try {
        return json.at(name).get<T>();
    } catch (const nlohmann::json::exception&) {
        throw std::runtime_error(std::string("llama runtime.json has invalid '") + name + "'");
    }
}
...
RuntimeConfig parse_runtime_config(const BundleReader& bundle) {
    ...
    try {
        ...
        throw std::runtime_error("llama invalid runtime.json: " + std::string(error.what()));
    }
    if (!json.is_object())
...
LlamaKvCacheNames make_kv_names(std::int32_t num_layers) {
    LlamaKvCacheNames names;
    for (std::int32_t layer = 0; layer < num_layers; ++layer) {
        names.cache_k.push_back(llama_layer_tensor_name("cache_k", layer));
        names.cache_v.push_back(llama_layer_tensor_name("cache_v", layer));
        names.present_k.push_back(llama_layer_tensor_name("present_k", layer));
        names.present_v.push_back(llama_layer_tensor_name("present_v", layer));
    }
    return names;
}
...
std::int32_t prefill_token_limit(const ITrtModule& module) {
    ...
    if (shape.size() != 1 || shape.front() <= 0)
        throw std::runtime_error("llama prefill engine has no valid token profile");
...
DecoderModules load_modules(const FamilyContext& context, const RuntimeConfig& config) {
    ...
    if (config.decoder_engine_layout == "split") {
        modules.decode = load_engine(
            context.backend, require_section(context.reader, "engine.plan"), "llama decoder");
        modules.prefill = load_engine(
            context.backend, require_section(context.reader, "prefill.plan"), "llama prefill");
        return modules;
    }
    ...
    if (dual.decode == nullptr || !dual.decode->ok() || dual.prefill == nullptr ||
        !dual.prefill->ok()) {
        throw std::runtime_error("llama dual-profile engine did not create both contexts");
    }
...
ITask* create(const FamilyContext& context) {
    ...
    return new LlamaTextGenerationPipeline(std::move(modules.decode), std::move(state),
                                           std::move(text_config), create_tokenizer(context.reader),
                                           std::move(modules.prefill), nullptr);
}
