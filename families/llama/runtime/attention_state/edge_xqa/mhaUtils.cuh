...
struct PagedKVCacheLayout
{
    ...
    {
        // MC pool: [pages, KV heads, slots, head dimension].
        return idxHeadGrp * getTokensPerPage(tokensPerPageLog2)
            + getTokenOffsetInPage(seqOffset, tokensPerPageLog2);
    }
        ...
        uint32_t tokenOffset, uint32_t tokensPerPageLog2)
    {
        ...
        return static_cast<uint32_t>(pageIdx) * pageStride + headOffset
            + getTokenOffsetInPage(tokenOffset, tokensPerPageLog2);
    }
...
{
    // MC adapter: independent immutable resources and independent page maps.
    GMemKVCacheHead* pool;
    GMemKVCacheHead* valuePool;
    KVCachePageIndex const* kvCachePageList; // shape: KVCachePageIndex[batchSize][beamWidth][2][maxNbPagesPerSeq].
    KVCachePageIndex const* valuePageList;
...
__device__ inline uint32_t getCacheSeqLen(KVCacheList<isPaged> const& cacheList, uint32_t idxReq)
{
    if constexpr (isPaged)
        return min(getSeqLen(cacheList.seqLenList, idxReq), cacheList.maxNbPagesPerSeq * TOKENS_PER_PAGE);
...
#pragma unroll
    for (uint32_t i = 0; i < nbLoadedPages; i++)
    {
        ...
        uint32_t const pageListOffset = idxReq * maxNbPagesPerSeq + idxPage;
        auto const* pages = isK ? cacheList.kvCachePageList : cacheList.valuePageList;
        uint32_t const count = isK ? cacheList.keyPageCount : cacheList.valuePageCount;
        int32_t const page = idxPage < nbPages ? pages[pageListOffset] : kBAD_PAGE_INDEX;
        ret[i] = page >= 0 && uint32_t(page) < count ? page : kBAD_PAGE_INDEX;
    }
    return ret;
}
