...
constexpr uint32_t tokensPerPageLog2 = exactLog2(tokensPerPage);
constexpr uint32_t minTokensPerPageLog2 = 4U;
constexpr uint32_t minTokensPerPage = 1U << minTokensPerPageLog2;
static_assert(tokensPerPage >= minTokensPerPage);
...
__device__ inline void applyMaskFromInput(Warp const& warp, WarpAcc& acc, MaskType const* mask, uint32_t rowOffset,
    ...
    uint32_t colOffset, uint32_t headOffset, uint32_t maskHeads, uint32_t capacity)
{
...
#pragma unroll
        ...
        {
            uint32_t const headToken = rowOffset + instM * m + idxQuad + i * 8;
            uint32_t const tokenRow = headToken / headGrpSize;
            uint32_t const maskHead = maskHeads == 1 ? 0 : headOffset + headToken % headGrpSize;
#pragma unroll
...
#pragma unroll
                for (uint32_t j = 0; j < InstAcc::cols; j++)
                {
                    ...
                    bool const visible = tokenRow < actualQSeqLen && col < nbValidCols
                        && mask[(maskHead * qSeqLen + tokenRow) * capacity + colOffset + col];
                    acc(m, n)(i, j) = visible ? __half2float(__float2half_rn(acc(m, n)(i, j))) : -INFINITY;
                }
            }
        }
    }
...
#if SPEC_DEC
    uint32_t headGrpSize, uint32_t idxHeadGrpOffset, uint32_t nbValidHeadTokens, uint32_t qSeqLen,
#else
    uint32_t idxHeadGrp,
#endif
    uint2 dstOffset, SharedMem::XSmemBuffer const& src, int* nonfiniteOutput)
{
...
#pragma unroll
    for (uint32_t i = 0; i < nbIters; i++)
    {
        ...
        uint32_t const idxHead = (idxHeadGrpOffset + headIdx) * qSeqLen + tokenIdx;
...
#pragma unroll
            for (uint32_t i = 0; i < outVec.size; ++i)
                nonfinite |= (__half_as_ushort(outVec[i]) & 0x7c00U) == 0x7c00U;
            if (nonfinite)
                atomicExch(nonfiniteOutput, 1);
        ...
        uint32_t* __restrict__ semaphores = nullptr, void* __restrict__ scratch = nullptr)
{
    ...
    __shared__ int mcNonfiniteOutput;
    ...
    mask += idxReq * cacheList.maskHeads * qSeqLen * cacheList.maxNbPagesPerSeq * TOKENS_PER_PAGE;
    // Preserve MC's empty-history and invalid-page behavior before any tiled loads.
    if (mc_xqa_contract_fallback(q, output, mask, cacheList, idxReq, idxHeadGrp,
            idxHeadTokenInGrp, nbValidHeadTokens, qSeqLen, nbKHeads, headGrpSize,
            reinterpret_cast<float*>(smemByteBuf)))
        return;
#else
    uint32_t const nbQHeads = nbKHeads * headGrpSize;
    ...
    if (warpIdx.z == 0)
    {
        ...
        auto const localQHeadTokenIdxMap
            = [nbQHeads, headGrpSize, reqSeqOffset, idxReq, idxHeadTokenInGrp, qSeqLen](uint32_t idxHeadTokenLocal) -> uint32_t
        {
            ...
            if constexpr (beamWidth == 1)
            {
                ...
                return headIdx * qSeqLen + tokenIdx;
            }
        };
        static_assert(nbValidRows <= warpTile.y);
        auto const srcBase = q;
        uint32_t const idxHeadTokenBeg = nbQHeads * reqSeqOffset + (idxHeadGrp * headGrpSize) * qSeqLen;
        auto const src = [&]()
        {
...
#if TOKENS_PER_PAGE != 0
    assert(modByPow2(ctaTile.x, tokensPerPageLog2) == 0);
    uint32_t const nbPages = divUpByPow2(cacheSeqLen, tokensPerPageLog2);
    constexpr uint32_t nbPagesPerCtaTile = divByPow2(ctaTile.x, tokensPerPageLog2);
    constexpr uint32_t nbPagesPerCtaTileLog2 = exactLog2(ctaTile.x) - tokensPerPageLog2;
#endif
    ...
    if (warpIdx.z == 0)
    {
        ...
        uint32_t idxPageBeg
            = nbPagesPerCtaTile * seqIterInit + divByPow2(warpIdx.x * warpTile.x, tokensPerPageLog2);
...
#if TOKENS_PER_PAGE != 0
            uint32_t const idxHeadBeg = getPagedHeadOffset(seqOffset, nbKHeads, idxHeadGrp, tokensPerPageLog2);
#if PAGED_KV_CACHE_LAYOUT == 1
            auto* const kCachePool = cacheList.kCacheVLLM;

#else
            auto* const kCachePool = cacheList.pool;
#endif
#if BEAM_WIDTH == 1
            // 2CTA head_dim=512 kernels read one slice from each paged cache head; other kernels read the full head.
            auto const src = makePagedHeadPtr<GMemCacheHead const, PaddedCacheHead const, nbPagesPerWarpTile>(
                kCachePool, pageIdx, nbKHeads, idxHeadBeg, splitCacheHeadByteOffset, tokensPerPageLog2);
#else
            IndexedHeadPtr<GMemCacheHead const, nbPagesPerWarpTile> const src
            {
                ...
                /*nbKHeads=*/nbKHeads,
                /*offset=*/idxHeadBeg,
                /*tokensPerPageLog2=*/tokensPerPageLog2
            };
#endif
#else
...
#else
            IndexedHeadPtr<GMemCacheHead const> const src{/*indices=*/smem.gemm0CacheIndir[warpIdx.x].data,
...
#pragma unroll(nbUnroll)
...
#endif
            {
                // MC supplies full logical visibility, including prefix holes,
                // per-head masks and speculative ancestors, on every tile.
                ...
                applyMaskFromInput(
                    ...
                    warpTileTokenBeg, idxHeadGrp * headGrpSize, cacheList.maskHeads,
                    cacheList.maxNbPagesPerSeq * TOKENS_PER_PAGE);
            }
#else
    ...
    else
    {
        ...
        uint32_t idxPageBeg
            = nbPagesPerCtaTile * seqIterInit + divByPow2(cacheVTileSeqLen * warpGrpIdx, tokensPerPageLog2);
        loadPages(idxPageBeg);
#else
        ...
        auto loadVTilePart
            = [&](uint32_t seqIter, uint32_t xIter, uint32_t vIter,
                  ...
                  uint32_t const idxHeadBeg = getPagedHeadOffset(seqOffset, nbKHeads, idxHeadGrp, tokensPerPageLog2);
#if PAGED_KV_CACHE_LAYOUT == 1
                  auto* const vCachePool = cacheList.vCacheVLLM;

#else
                  auto* const vCachePool = cacheList.valuePool;
#endif
#if BEAM_WIDTH == 1
                  // 2CTA head_dim=512 kernels read one slice from each paged cache head; other kernels read the full head.
                  auto const src = makePagedHeadPtr<GMemCacheHead const, PaddedCacheHead const, nbPagesPerVTile>(
                      vCachePool, pageIdx, nbKHeads, idxHeadBeg, splitCacheHeadByteOffset, tokensPerPageLog2);
#else
                  IndexedHeadPtr<GMemCacheHead const, nbPagesPerVTile> const src
                  {
...
#else
                  IndexedHeadPtr<GMemCacheHead const> const src{
                      /*indices=*/smem.gemm1CacheIndir[grpLoadV ? warpGrpIdx : warpIdx.x].data,
                      /*pointer=*/cacheList.data,
                      /*offset=*/idxHeadBeg,
                      /*beamStride=*/cacheList.capacity * nbKHeads * 2};
            ...
            // }
                  ...
                  {
                      uint32_t const nbXItersPerPageLog2 = tokensPerPageLog2 - exactLog2(xIterSeqStride);
                      uint32_t const nbXItersPerPage = 1U << nbXItersPerPageLog2;
                      assert(nbXItersPerPage <= nbXItersPerCtaTile);
                      if (modByPow2(xIter, nbXItersPerPageLog2) == nbXItersPerPage - 1 && vIter == nbVItersPerXIter - 1
                          && (idxBeam == beamWidth - 1 || isConvergedTile(seqIter)))
                      {
                          auto const step = 1;
                          idxPageBeg += (modByPow2(idxPageBeg, nbPagesPerCtaTileLog2) == nbPagesPerCtaTile - 1
                  ...
                  else
                  {
                      ...
                      {
                          auto const step = divByPow2(xIterSeqStride, tokensPerPageLog2);
                          idxPageBeg += (modByPow2(idxPageBeg, nbPagesPerCtaTileLog2) + step >= nbPagesPerCtaTile
                                  ? nbPagesPerCtaTile * (nbSubSeqPerSeq - 1) + step
                                  : step);
                          loadPages(idxPageBeg);
                      }
...
#pragma unroll
        ...
        { // otherwise rcpRowSum will be NAN.
            ...
            for (uint32_t i = 0; i < rcpRowSum.size; ++i)
                if (globalRowSum[i] == 0.0F)
                    rcpRowSum[i] = 0.0F;
#if LOW_PREC_OUTPUT
...
#pragma unroll
        ...
        if (warpGrpIdx == 0)
        {
            ...
            copyOutputToGlobalMem(warp, &output[reqSeqOffset * nbQHeads], nbQHeads, headGrpSize,
                (idxHeadGrp * headGrpSize), nbValidHeadTokens, qSeqLen,
            ...
            copyOutputToGlobalMem(warp, &output[nbQHeads * beamWidth * idxReq], nbQHeads, idxHeadGrp,
                uint2{warpTile.x * warpIdxInGrp, nbValidRows * warpIdx.y}, *smemOutTile, &mcNonfiniteOutput);
#endif
        }
    }

    // Producer warps must exit as in the donor: joining a later barrier 0
    // would release the consumer-only merge barriers prematurely. Only the
    // consumer half performs the optional repair after all output stores.
    if (warpIdx.z == 0)
        return;
    __syncthreads();
    if (mcNonfiniteOutput)
        mc_xqa_contract_fallback(q, output, mask, cacheList, idxReq, idxHeadGrp,
            idxHeadTokenInGrp, nbValidHeadTokens, qSeqLen, nbKHeads, headGrpSize,
            reinterpret_cast<float*>(smemByteBuf), true);
}
...
#if 0 // MC supplies its own AOT launcher; no donor host runtime dependencies.
...
#if TOKENS_PER_PAGE != 0
    uint32_t const maxNbPagesPerSeq = divByPow2(maxSeqLen, tokensPerPageLog2);
#if PAGED_KV_CACHE_LAYOUT == 1
